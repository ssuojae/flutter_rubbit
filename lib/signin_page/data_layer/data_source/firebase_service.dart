...
final class FirebaseService {
  ...
  Future<kakao.OAuthToken> _requestKakaoAdditionalScopesIfNeeded(kakao.OAuthToken token, kakao.User kakaoUser) async {
    List<String> scopes = [];
  ...
  Future<void> _saveAppleUserToFirestore(
      firebase_auth.User? user, AuthorizationCredentialAppleID appleCredential) async {
    if (user == null) return;
    ...
    print('Apple user info saved to Firestore: ${user.uid}, ${appleCredential.givenName}, ${appleCredential.email}');
  }
  ...
  Future<void> _saveKakaoUserToFirestore(firebase_auth.User? user, kakao.User kakaoUser) async {
    ...
    print(
        'Kakao user info saved to Firestore: ${user.uid}, ${kakaoUser.kakaoAccount?.profile?.nickname}, ${kakaoUser.kakaoAccount?.email}, ${kakaoUser.kakaoAccount?.profile?.profileImageUrl}');
  }
